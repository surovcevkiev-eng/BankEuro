...
<section class="gray-page">
  <div class="container products-pege col-catalog">
		 ...
		 <div class="modal-inner">
			 ...
			 <div class="modal-inner-container modal-scroll-y">
			  <div class="features-box">
					{* Фильтр по свойствам *}
					  ...
					  <table id="features" class="features-table">
						...
						<tr>
						  ...
						  <td class="feature-values"><a href="{url params=[$f->id=>null, page=>null]}" {if !$smarty.get.$key}class="selected"{/if}>Всі</a> {foreach $f->options as $o} <a href="{url params=[$f->id=>$o->value, page=>null]}" {if $smarty.get.$key == $o->value}class="selected"{/if}>{$o->value|escape}</a> {/foreach} </td>
						</tr>
						{/foreach}
					  </table>
					  {/if}	
				  ...
				  <div class="padding2-0 border-bottom2x all-brands-box">{if $category->brands}<div id="brands" class="brands-box"><a href="catalog/{$category->url}" {if !$brand->id}class="selected"{/if}>Всі бренди</a>{foreach $category->brands as $b}{if $b->image}<a data-brand="{$b->id}" href="catalog/{$category->url}/{$b->url}"><img src="{$config->brands_images_dir}{$b->image}" alt="{$b->name|escape}"></a>{else}<a data-brand="{$b->id}" href="catalog/{$category->url}/{$b->url}" {if $b->id == $brand->id}class="selected"{/if}>{$b->name|escape}</a>{/if}{/foreach}</div>{/if}</div>	
    ...
    <div class="products-pege-main">
      ...
      <!-- Хлебные крошки /-->
      <div id="path" class="bread-crumbs"> <a href="/">Главная</a> {if $category}
			 ...
			 {* Сортировка *}
			  <form>
				...
				<select class="sort-box"  name="sort" size="1">
					<option {if $sort=='position'}selected{/if} value="{url sort=position page=null}">за замовчуванням</option>
					<option {if $sort=='price'}selected{/if} value="{url sort=price page=null}">за зростанням ціни</option>
					<option {if $sort=='name'}selected{/if} value="{url sort=name page=null}">за назвою</option>
				</select>
			</form>
		 </div>
	 </div> 		
