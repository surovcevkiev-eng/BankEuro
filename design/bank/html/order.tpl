...
{$meta_title = "Ваше замовлення №`$order->id`" scope=parent}
<div class="container container-min">
  <h1 class="padding2-0">Ваше замовлення №{$order->id} 
    {if $order->status == 0}прийнято{/if}
    {if $order->status == 1}в обробці{elseif $order->status == 2}виконаний{/if}
    {if $order->paid == 1}, оплачений{else}{/if} </h1>
  {* Список покупок *}
