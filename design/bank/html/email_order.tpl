...
<h1 style="font-weight:normal;font-family:arial;">
	<a href="{$config->root_url}/order/{$order->url}">Ваше замовлення №{$order->id}</a>
	на суму {$order->total_price|convert:$currency->id}&nbsp;{$currency->sign}
	{if $order->paid == 1}оплачений{else}ще не оплачений{/if},
	{if $order->status == 0}чекає на обробку{elseif $order->status == 1}в обробці{elseif $order->status == 2}виконаний{elseif $order->status == 3}скасовано{/if}
</h1>
...
<table cellpadding="6" cellspacing="0" style="border-collapse: collapse;">
	...
	<tr>
		...
		<td style="padding:6px; width:250; padding:6px; background-color:#f0f0f0; border:1px solid #e0e0e0;font-family:arial;">
			...
			<br>
			<a href="{$config->root_url}/order/{$order->url}/{$purchase->variant->attachment}"><font color="green">Завантажити {$purchase->variant->attachment}</font></a>
			{/if}
		</td>
...
<a href="{$config->root_url}/order/{$order->url}" target="_blank">{$config->root_url}/order/{$order->url}</a>
<br>