...
{$meta_title = "`$product->name|escape` `$product->brand->name|escape`, ціна `$product->variant->price|convert` `$currency->sign|escape` | Goldrwsbank" scope=parent}
...
<section class="header-product gray-page padding2-0">
  <div class="container">
    <div class="product-header-menu">
		...
		<!-- Хлебные крошки /-->
		<div id="path" class="product-header-bread bread-crumbs padding0"> <a class="bread-crumbs--active" href="./">Главная</a> {foreach $category->path as $cat}
		  ...
		  / <a class="bread-crumbs--active">{$product->name|escape}</a>
		</div>
		<!-- Хлебные крошки #End /--> 
  </div>
 </div>	  
...
<!--									  	<input type="submit" class="button" value="Зробити резерв" data-result-text="В корзині">-->
	 ...
	 <div id="features" class="product-features paddingtop-3">
	 	{if $product->features} 
		  <h2>Характеристики <span>{$product->name|escape}</span></h2>
      ...
      <div id="description" class="product-description paddingtop-2"> 
		{if $product->body} 
			<h3>Опис <span>{$product->name|escape}</span></h3>
		{/if}
			...
			<div class="comments-box__item comment-form">
			  ...
			  <form method="post" class="gray-page padding2">
				...
				<div class="comments-box-form">
				  ...
				  <input class="input_name" type="text" id="comment_name" name="name" value="{$comment_name}" data-format=".+" data-notice="Ваше Ім'я">
				  <label for="comment_text">Ваш відгук</label>
				  <textarea class="comment-textarea" rows="5" id="comment_text" name="text" data-format=".+" data-notice="Ваш відгук">{$comment_text}</textarea>
